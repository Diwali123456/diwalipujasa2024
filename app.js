...
import('emailjs').then((emailjsModule) => {
  ...
  const emailConfig = {
    user: 'diwalipujasa2024@gmail.com',
  ...
  async function sendEmail(urn, userEmail,userName) {
    try {
      const message = {
        from: 'Registration Team <diwalipujasa2024@gmail.com>',
        to: `${userName} <${userEmail}>`,
        cc: '<diwalipujasa2024@gmail.com>',
        subject: 'Registration Confirmation - Diwali Puja Cap Town South Africa 2024',
        attachment: [
          { data: `<html>
  ...
  app.post("/registration", async function (req, res) {
    ...
    for (let i = 1; i <= numYuvas; i++) {
     ...
     // const yuvaStaying = req.body[`yuvaStaying${i}`];
      syDetails.push({ name: yuvaName, address: yuvaAddress, gender: yuvaGender, mobile: req.body.payerMobile, category: "Y", urn:urn, age:yuvaAge });
      yuvaDetails.push({ name: yuvaName, address: yuvaAddress, gender: yuvaGender, age:yuvaAge });
    }
    ...
    for (let i = 1; i <= numAdults; i++) {
     ...
     // const adultStaying = req.body[`adultStaying${i}`];
      ...
      syDetails.push({ name: adultName, address: adultAddress, gender: adultGender, mobile: req.body.payerMobile, category: "A", urn:urn, age:adultAge });
      adultDetails.push({ name: adultName, address: adultAddress, gender: adultGender, age:adultAge });
    }
    ...
    for (let i = 1; i <= numBals; i++) {
      ...
      syDetails.push({ name: balName, address: balAddress, gender: balGender, mobile: req.body.payerMobile, category: "C", urn:urn,age:balAge });
      balDetails.push({ name: balName, address: balAddress, gender: balGender, age:balAge  });
    }
